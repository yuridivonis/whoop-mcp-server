...
- **Day labels:** days were labelled by the UTC date a Whoop cycle started. So in timezones east of UTC, every row showed the night before. Days now use your local date, from the timezone Whoop records, and a server running west of UTC no longer shifts them either. (#7)
...
- **Workouts tool:** `get_workouts` lists recent workouts with activity, duration, strain, heart rate, calories, and time in heart-rate zones. (#7)
