...
- **Stay up to date:** run the `:1` image, which picks up every 1.x release when you redeploy, or keep your fork updated (GitHub's **Sync fork** button). Watch this repository's releases (**Watch → Custom → Releases**) to hear about security fixes. Unless `UPDATE_CHECK=false`, `get_today` also mentions a newer version once it's out.
