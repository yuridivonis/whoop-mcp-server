...
- **GitHub, for the update check:** once a day, the server asks GitHub for this project's latest release number, so it can tell you when a newer version is out. The request carries nothing about you or your data, only the server's address as any web request does. The operator can turn it off with `UPDATE_CHECK=false`.
